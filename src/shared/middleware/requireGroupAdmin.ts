...
export const requireGroupAdmin = async (
  ...
  try {
    const q = `SELECT id FROM group_members WHERE group_id = $1 AND user_id = $2 AND role = 'ADMIN' AND status = 'ACTIVE'`;
    const r = await query(q, [groupId, userId]);
    if (r.rowCount === 0)
      return res
        .status(403)
        .json({ success: false, message: "Forbidden: admin required" });
    return next();
  } catch (err: any) {
    return res
      .status(500)
